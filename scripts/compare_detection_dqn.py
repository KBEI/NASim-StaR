...
def save_plots(results_no_det, results_with_det, out_dir):
    try:
    ...
    plt.plot(
        [ep["episode_return"] for ep in results_no_det["episodes"]],
        label="no detection",
    )
    plt.plot(
        [ep["episode_return"] for ep in results_with_det["episodes"]],
    ...
    plt.figure(figsize=(10, 4))
    plt.plot(
        [ep["episode_steps"] for ep in results_no_det["episodes"]],
        label="no detection",
    )
    plt.plot(
        [ep["episode_steps"] for ep in results_with_det["episodes"]],
        label="with detection",
    )
    plt.xlabel("Episode")
...
def save_markdown(summary, out_path):
    ...
    lines = [
        "# DQN Detection Comparison Report",
        "",
    ...
    for condition in ["no_detection", "with_detection"]:
    ...
    lines.append("")
    lines.extend(_md_best_episode("No Detection", summary["no_detection"]["best_episode"]))
    lines.extend(_md_best_episode("With Detection", summary["with_detection"]["best_episode"]))
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare DQN training with/without detection."
    )
    parser.add_argument("--training_steps", type=int, default=20000)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--out_dir", type=str, default="runs/compare_detection")
    ...
    tiny_det_path = os.path.join(
        repo_root, "scenarios", "benchmark", "tiny-with-detection.yaml"
    )

    env_no_detection = nasim.make_benchmark(
        "tiny",
        seed=args.seed,
        fully_obs=True,
        flat_actions=True,
    ...
    results_no_det = train_collect(
        env_no_detection, args.training_steps, args.seed, dqn_kwargs
    )
    results_with_det = train_collect(
        env_with_detection, args.training_steps, args.seed, dqn_kwargs
    )
    ...
    summary = {
        ...
        "seed": args.seed,
        "no_detection": {
            "eval_return": results_no_det["eval_return"],
            "eval_steps": results_no_det["eval_steps"],
            "eval_goal": results_no_det["eval_goal"],
            "avg_return_last10": float(
                np.mean(
                    [ep["episode_return"]
                     for ep in results_no_det["episodes"][-10:]]
                ) if len(results_no_det["episodes"]) >= 10 else np.mean(
                    [ep["episode_return"] for ep in results_no_det["episodes"]]
                )
            ),
            "episodes": results_no_det["episodes"],
            "best_episode": results_no_det["best_episode"],
        },
        "with_detection": {
            "eval_return": results_with_det["eval_return"],
            "eval_steps": results_with_det["eval_steps"],
            "eval_goal": results_with_det["eval_goal"],
            "avg_return_last10": float(
                np.mean(
                    [ep["episode_return"]
                     for ep in results_with_det["episodes"][-10:]]
                ) if len(results_with_det["episodes"]) >= 10 else np.mean(
                    [ep["episode_return"] for ep in results_with_det["episodes"]]
                )
            ),
            "episodes": results_with_det["episodes"],
            "best_episode": results_with_det["best_episode"],
        },
    }
    ...
    if not args.no_plots:
        save_plots(results_no_det, results_with_det, out_dir)

    csv_no_det = os.path.join(out_dir, f"episodes_no_detection_{timestamp}.csv")
    csv_with_det = os.path.join(out_dir, f"episodes_with_detection_{timestamp}.csv")
    save_csv(results_no_det, "no_detection", csv_no_det)
    save_csv(results_with_det, "with_detection", csv_with_det)
    ...
    print(f"Saved CSV to: {csv_no_det}")
    print(f"Saved CSV to: {csv_with_det}")
