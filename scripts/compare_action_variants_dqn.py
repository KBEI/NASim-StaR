...
def save_markdown(summary, out_path):
    ...
    lines = [
        "# DQN Action Variant Comparison Report",
        "",
    ...
    lines.extend(_md_best_episode("Custom", summary["custom"]["best_episode"]))
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare DQN training with baseline vs custom actions."
    )
    parser.add_argument("--training_steps", type=int, default=20000)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--out_dir", type=str, default="runs/compare_actions")
    parser.add_argument(
        "--baseline_scenario",
        type=str,
        default=os.path.join("scenarios", "benchmark", "small.yaml"),
    )
    parser.add_argument(
        "--custom_scenario",
        type=str,
        default=os.path.join(
            "scenarios", "benchmark", "small-multi-actions.yaml"
        ),
    )
    ...
    csv_custom = os.path.join(out_dir, f"episodes_custom_{timestamp}.csv")
    save_csv(results_baseline, "baseline", csv_baseline)
    save_csv(results_custom, "custom", csv_custom)
