...
def save_plots(results_base, results_time, out_dir):
    try:
    ...
    plt.plot(
        [ep["episode_return"] for ep in results_time["episodes"]],
        label="time",
    )
    plt.xlabel("Episode")
    ...
    plt.plot(
        [ep["episode_steps"] for ep in results_time["episodes"]],
        label="time",
    )
    plt.xlabel("Episode")
...
def save_markdown(summary, out_path):
    ...
    lines.extend(_md_best_episode("Time", summary["time"]["best_episode"]))
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare DQN training with/without time."
    )
    parser.add_argument("--training_steps", type=int, default=20000)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--out_dir", type=str, default="runs/compare_time")
    ...
    base_path = os.path.join(
        repo_root, "scenarios", "benchmark", "tiny.yaml"
    )
    time_path = os.path.join(
        repo_root, "scenarios", "benchmark", "tiny-with-time.yaml"
    )
    ...
    env_time = nasim.load(
        time_path, fully_obs=True, flat_actions=True, flat_obs=True
    )

    results_base = train_collect(
        env_base, args.training_steps, args.seed, dqn_kwargs
    )
    results_time = train_collect(
        env_time, args.training_steps, args.seed, dqn_kwargs
    )
    ...
    summary = {
        ...
        "time": {
            "eval_return": results_time["eval_return"],
            "eval_steps": results_time["eval_steps"],
            "eval_goal": results_time["eval_goal"],
            "avg_return_last10": float(
                np.mean(
                    [ep["episode_return"]
                     for ep in results_time["episodes"][-10:]]
                ) if len(results_time["episodes"]) >= 10 else np.mean(
                    [ep["episode_return"] for ep in results_time["episodes"]]
                )
            ),
            "episodes": results_time["episodes"],
            "best_episode": results_time["best_episode"],
        },
    }
    ...
    if not args.no_plots:
        save_plots(results_base, results_time, out_dir)

    csv_base = os.path.join(out_dir, f"episodes_base_{timestamp}.csv")
    csv_time = os.path.join(out_dir, f"episodes_time_{timestamp}.csv")
    save_csv(results_base, "base", csv_base)
    save_csv(results_time, "time", csv_time)
